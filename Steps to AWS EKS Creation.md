 ...
 * Using `eksctl create cluster -f ekscluster.yml`, AWS provisions the control plane, VPC, networking, and worker nodes.
...
### EKS Accesss
  ...
  * Even if a user has AdministratorAccess in AWS, they cannot view Kubernetes resources unless they are added to the EKS Access Entries or mapped through `aws-auth`.

---

 * AWS CLI command lists all EC2 instance types that AWS currently marks as `Free Tier eligible` in your account/region.
 * Select `instance type` and update instance in `ekscluster.yaml` managedNodeGroups
```hcl
