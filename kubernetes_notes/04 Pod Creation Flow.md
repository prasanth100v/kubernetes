...
2. ⚙️ **API Server receives the request**, validates it, and stores pod details in **etcd** (a key-value database).

3. 🗄️ **etcd stores the Pod configuration data**, but no node is assigned yet.
...
6. 🖥️ **Kubelet on the assigned node** pulls the container image and starts the pod.
...
3. 🚀 **Kubelet starts the container**
    - Kubelet requests the **container runtime** (containerd / Docker) to:
      ...
      - ▶️ Create and start the container
...
🚀 User Request
   ↓
⚙️ API Server (Validate Request)
   ↓
🗄️ etcd (Store Pod Data)
   ↓
🧠 Scheduler (Select Node)
   ↓
🔄 API Server Updates Assignment
   ↓
🖥️ Kubelet (Start Container)
   ↓
