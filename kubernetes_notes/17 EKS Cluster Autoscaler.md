...
Cluster Autoscaler automatically adds or removes nodes in the cluster
based on:
...
-   Integrated with node groups (e.g., EKS Managed Node Group or ASG)

------------------------------------------------------------------------

## 🌲 What is EKS Cluster Autoscaler?

EKS Cluster Autoscaler automatically adds or removes EC2 worker nodes in
our EKS node group when pods can't be scheduled due to insufficient
resources.
...
-   Your microservice gets a traffic spike. HPA increases pod replicas,
    but there is no space on current nodes.
-   Cluster Autoscaler launches new EC2 nodes to fit the new pods.
-   Traffic is low at night, and your app only needs 2 nodes instead of
    6.
-   Cluster Autoscaler deletes unused nodes (if pods can run elsewhere),
    saving cost.
...
-   At least one managed node group (Amazon EC2)
...
Cluster Autoscaler uses IAM Roles for Service Accounts (IRSA), which
needs OIDC.
...
✅ This enables your cluster to authenticate IAM roles through service
accounts.
...
✅ This policy gives permissions to interact with EC2 Auto Scaling
Groups.
...
✅ This connects the IAM role with the Kubernetes service account
(IRSA).
...
Go to **AWS Console → EC2 → Auto Scaling Groups → your node group → Add
Tags**
...
✅ These tags allow Cluster Autoscaler to discover and scale your node
group.
...
This tells the Cluster Autoscaler:

❌ **Do NOT evict this pod** when trying to scale down a node.

Without this annotation: Cluster Autoscaler might evict the pod during
scale-down if it thinks there is space elsewhere.
...
4.  Tag your Node Group ASG\
5.  Deploy and configure Cluster Autoscaler\
6.  Watch logs and test scaling
