 ...
 * 🔴 **Too many pods? Verify:**
   ...
   * ⚠️ The labels in `template.metadata.labels` **must match** the `selector.matchLabels`.
...
- ❗ Do not forget:
  ...
  - 🧹 When you manually delete the old pods, the ReplicaSet creates new ones with the updated image. (⚠️ ReplicaSet `does NOT update the image automatically`)

---

## 🔧 ⚙️ ReplicaSet Commands
```bash
kubectl apply -f replicaset.yaml                   # 🚀 Create a ReplicaSet from YAML

kubectl get rs                                     # 📋 List all ReplicaSets

kubectl describe rs <replicaset-name>              # 🔍 Get detailed information including events and selector labels

kubectl scale rs <replicaset-name> --replicas=5         # 🔢 Change the number of replicas

kubectl delete rs <replicaset-name>                     # ❌ Delete a ReplicaSet

kubectl get pods --selector=app=nginx                   # 📦 View Pods created by a ReplicaSet

kubectl get pods --show-labels                          # 🏷 List all pods along with their labels

kubectl get pods -w                                     # 👀 Watch real-time updates of pods

kubectl edit rs <replicaset-name>                       # ✏️ Edit a ReplicaSet
```
